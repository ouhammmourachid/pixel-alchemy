...
def random_flip(lr_img, hr_img):
    ...
    return tf.cond(
        flip_chance < 0.5, lambda: (lr_img, hr_img), lambda:
        (tf.image.flip_left_right(lr_img), tf.image.flip_left_right(hr_img))
    )
...
def random_lr_jpeg_noise(lr_img, hr_img, min_jpeg_quality=50, max_jpeg_quality=95):
    ...
    return tf.cond(
        jpeg_noise_chance < 0.5, lambda: (lr_img, hr_img), lambda:
        (tf.image.random_jpeg_quality(lr_img, min_jpeg_quality, max_jpeg_quality), hr_img)
    )
