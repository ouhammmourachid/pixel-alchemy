...
def create_training_dataset(dataset_parameters, train_mappings, batch_size):
    lr_dataset = image_dataset_from_directory_or_url(
        dataset_parameters.save_data_directory, dataset_parameters.train_directory, dataset_parameters.train_url
    )
    hr_dataset = image_dataset_from_directory_or_url(
        dataset_parameters.save_data_directory, 'DIV2K_train_HR', hr_train_url
    )
...
def create_validation_dataset(dataset_parameters):
    lr_dataset = image_dataset_from_directory_or_url(
        dataset_parameters.save_data_directory, dataset_parameters.valid_directory, dataset_parameters.valid_url
    )
    hr_dataset = image_dataset_from_directory_or_url(
        dataset_parameters.save_data_directory, 'DIV2K_valid_HR', hr_valid_url
    )
